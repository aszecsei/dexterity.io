...
RSpec.describe User, type: :model do
  ...
  it "connects to the correct role" do
    user1 = FactoryGirl.create(:user)
    proj = FactoryGirl.create(:project, name: "John's Helper")
    role = FactoryGirl.create(:role, project_id: 1, name: 'Scrum Master')
    FactoryGirl.create(:workingon, project: proj, user: user1, role: role)
  ...
  it "can properly distinguish projects and roles" do
    ...
    proj1 = FactoryGirl.create(:project, name: "John's Helper")
    proj2 = FactoryGirl.create(:project, name: "Ann's Helper")
    role1 = FactoryGirl.create(:role, project_id: 1, name: 'Scrum Master')
    role2 = FactoryGirl.create(:role, project_id: 2, name: 'Developer')
    role3 = FactoryGirl.create(:role, project_id: 1, name: 'Developer')
    role4 = FactoryGirl.create(:role, project_id: 2, name: 'Scrum Master')
    FactoryGirl.create(:workingon, project: proj1, user: user1, role: role1)
    FactoryGirl.create(:workingon, project: proj1, user: user2, role: role3)
    FactoryGirl.create(:workingon, project: proj2, user: user1, role: role2)
    FactoryGirl.create(:workingon, project: proj2, user: user2, role: role4)
    expect(user1.projects[0].name).to eq("John's Helper")
